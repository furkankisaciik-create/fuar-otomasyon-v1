...
# --- 1. URL SEKMESİ (CLOUD-BYPASS MOTORU) ---
with t1:
    ...
    if st.button("🔍 URL'den Firmaları Çek"):
        if url_input:
            # Gerçek bir kullanıcı gibi davranan gelişmiş başlıklar
            headers = {
            ...
            try:
                with st.spinner("Site koruması aşılıyor, lütfen bekleyin..."):
                    # Session kullanarak çerez yönetimini simüle et
                    session = requests.Session()
                    res = session.get(url_input, headers=headers, timeout=25, verify=False)
                    html_content = res.text
                    
                    # MÜSİAD sitesine özel seçici (Büyük/Küçük harf duyarsız ve tüm karakterler)
                    # Firma isimleri genellikle <a> içindeki title veya metin olarak bulunur
                    isimler = re.findall(r'<a[^>]*>(.*?)</a>', html_content, re.IGNORECASE)
                    
                    temiz_liste = []
                    yasakli_kelimeler = ['giriş', 'kayıt', 'menü', 'iletişim', 'fuar', 'expo', 'müsiad', 'detay', 'tıklayın', 'ara', 'sayfa']
                    
                    for isim in isimler:
                        temiz = re.sub('<.*?>', '', isim).strip() # HTML temizle
                        # Filtreleme: Uzunluk ve içerik kontrolü
                        if 4 < len(temiz) < 60:
                            if not any(kelime in temiz.lower() for kelime in yasakli_kelimeler):
                                if temiz[0].isalpha(): # Sadece harf ile başlayanları al
                                    temiz_liste.append(temiz)
                    
                    # Mükerrerleri sil ve alfabetik diz
                    st.session_state['ana_liste'] = sorted(list(set(temiz_liste)))
                    
                    if len(st.session_state['ana_liste']) > 0:
                        st.success(f"🚀 Zafer! {len(st.session_state['ana_liste'])} firma ismi başarıyla çekildi.")
                        st.rerun()
                    else:
                        st.error("Site hala veri vermiyor. Bu durumda lütfen MANUEL sekmesini kullanın.")
